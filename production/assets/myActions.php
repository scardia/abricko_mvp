...
function getTopTen1()
{
    ...
    $maxx=$_REQUEST['maxx'];
    $maxy=$_REQUEST['maxy'];
    $qry="SELECT distinct `title`, `latitude`, `longitude`, `yieldValue`, `imgLink`, `url` FROM `st_listings_sale` WHERE (`latitude` BETWEEN ".$miny." AND ".$maxy.") AND (`longitude` BETWEEN ".$minx." AND ".$maxx.") and `yieldValue`>0 and `yieldValue`<36 ORDER by `yieldValue` DESC LIMIT 0,10";
    //return $qry;
    $result=mysqli_query($con, $qry);
    $data=array();
    if (mysqli_num_rows($result) >0) {
